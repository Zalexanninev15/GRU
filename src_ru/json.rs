...
fn create_github_request(url: &str, ua: &str, api_key: Option<&str>) -> Request<()> {
    ...
    builder.body(()).expect("Ошибка создания запроса")
}
...
fn fetch_and_parse_releases(url: &str, ua: &str, api_key: Option<&str>) -> Value {
    ...
    let json = request
        .send()
        .expect("GitHub API: ошибка 404")
        .text()
        .expect("GitHub API: Json потерян");
    serde_json::from_str(&json).expect("GitHub API: Ошибка парсинга json")
}
...
fn parse_text(json: &str, word: &str) -> (String, String) {
    let release: Value = serde_json::from_str(json).expect("GitHub API: Ошибка парсинга json");
