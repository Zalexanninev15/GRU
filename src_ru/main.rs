...
fn main() {
    ...
    {
        println!("Файлы '7z.exe' и/или '7z.dll' не найдены!");
    ...
    if arguments.len() >= 3 {
        let arguments = arguments
            ::parse(arguments)
            .expect("Ошибка в аргументе! Просто запустите GRU для отображения справки");
        ...
        if s.is_err() {
            println!("Ошибка подключения к GitHub!");
            update_now = false;
        }
        ...
        if debug_mode {
            ...
            press_btn_continue::wait("[Debug] Нажмите Enter для продолжения...").unwrap();
        }
        ...
        if debug_mode {
            ...
            press_btn_continue::wait("[Debug] Нажмите Enter для продолжения...\n").unwrap();
        }

        // Checker for сurrent and new version
        if create_only_version_file {
            println!("\nТекущая версия приложения: {}", &v_list_version);
            main_func::set_new_version(&v_list_version);
            update_now = false;
            println!("\nПриложение имеет последнюю версию!");
        } else {
            ...
            if version_status_code != 0 && create_only_version_file == false {
                if v_list_version != "" {
                    println!("\nНовая версия {} доступна для скачивания!", v_list_version);
                } else {
                    update_now = false;
                    println!(
                        "\nНовых версий не обнаружено, если вы уверены, что новая версия существует, то используйте аргументы  '--debug' или '--pre'."
                    );
                }
                if version_status_code == -1 {
                    println!(
                        "\nОднако это может быть неточно, поскольку версия, возможно, была определена неправильно!"
                    );
                }
            } else {
                update_now = false;
        ...
        if update_now {
            ...
            if debug_mode {
                press_btn_continue::wait("Нажмите Enter для продолжения...").unwrap();
                ...
                )
            {
                if metadata.is_file() {
                    ...
                    if first_launch {
                        ...
                        println!("Обновление успешно завершено!");
                    }
                } else {
                    println!("Файл по какой-то причине не был загружен!");
                }
            }
        } else {
            println!("Мне не нужно ничего скачивать.");
            println!(
                "Или проверьте аргументы, если вы уверены, что загрузка должна была точно начаться."
            );
        }
        if debug_mode {
            println!("\n[Debug] State 4");
